...
class ScicatInfoView(AuthJSONRestView):
    def get(self, request, scicat_name=None, sid=None):
        if sid:
            path = "scicat/id/" + str(sid) + "/"
        elif scicat_name:
            path = "scicat/" + scicat_name + "/"
        return acct_adapter.get_resource(path)

# /api/account/image/<scicat_name>/photo/
# /api/account/image/id/<iid>/photo/
class ImageInfoView(AuthJSONRestView):
    def get(self, request, image_name=None, iid=None):
        if iid:
            path = "image/id/" + str(iid) + "/photo/"
        elif image_name:
            path = "image/" + image_name + "/photo/"
        return acct_adapter.get_image(path)
